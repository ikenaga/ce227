...
``` 
- Seja $x_1, ... x_n$ uma a.a. de uma distribuição normal de média $\mu$ conhecida
e variância $\sigma^2$ desconhecida. Considere a priori $[\sigma^2] \propto 1/\sigma^2$.
- Obtenha a expressão da verossimilhança do modelo.
- Obtenha a expressão da distribuição a posteriori.
- É possível identificar a posteriori do modelo como alguma distribuição conhecida?
- Considere que foi tomada a amostra dada pelos valores a seguir, e que $\mu = 10$. Obtenha a expressão
da posteriori.
- Indique (com comandos do R ou de alguma outra forma) como resumos pontuais e intervalares desta distribuição a posteriori poderiam ser obtidos para fins de inferência
...
- As funções de verossimilhança($L(\sigma^2)$), 
...
o valor da soma de quadrados e tamanho da amostra *n* no lugar dos dados.}.
```{r }
...
[h!]
```{r vero-sigma2,fig=T,width=9,height=6}
...
- A expressão anterior, vista como uma função de $\sigma^2$, 
...
```
- 
- Resumos pontuais da posteriori.
- Média da posteriori. Neste caso se tem a expressão analítica e que portanto deve ser utilizada. Entretanto, como em muitos casos pode não ser disponível, ilustra-se também a obtenção por integração numérica e por simulação. 
- Expressão analítica
...
mean(sim)
``` 
- Moda da posteriori. Assim como no caso da média a expressão analítica da moda é conhecida mas ilustra-se também a obtenção por otimização numérica e por simulação.
- Expressão analítica.
\[ E[\sigma^2|y] = \frac{1}{\beta(\alpha+1)} = \Sexpr{format(1/(b.post*(a.post+1)), dig=3)} \]
```{r }
...
``` 
- Simulação. Utiliza-se aqui um algorítmo simples tomando-se o ponto de máximo de uma suavização da densidade.\footnote{veja outro algorítmo mais adiante com o uso da função `hdrcde::hdr()`.}
```{r }
...
``` 
[h!]
```{r echo=F,fig=T, width=8,height=6}
...
``` 
- Resumos intervalares (usando $1-\alpha = 0,95$)
- Intervalo de credibilidade -- quantis
- Por otimização numérica
```{r }
...
``` 
- HDR obtido a partir de simulações da posteriori.
```{r }
require(hdrcde)
...
1/APnum$hessian
``` 
Uma forma alternativa mais simples e direta de definir no `R` a log-posteriori
...
algorítmo de *Cadeias de Markov via Monte Carlo* (MCMC) para obter amostras da posteriori.
O algorítmo de Metropolis-Hastings para amostrar de uma distribuição $\pi(\theta)$ segue os seguintes passos:
- Toma-se um valor inicial $\theta_0$ (arbitrário) para o parâmetro.
- Escolhe-se uma função $q(\theta_*|\theta_{t-1})$ para sortear um novo valor $\theta_*$ ``proposto'' para  o parâmetro.
- Geram-se valores em uma ``cadeia'' (*suficientemente grande*) segundo as regras: 
- para cada valor proposto calcula-se $\alpha = {\rm min}\left\{1, \frac{\pi(\theta_*) q(\theta_{t-1}|\theta_*)}{\pi(\theta_{t-1}) q(\theta_*|\theta_{t-1})}\right\} ,$
- aceita-se que a cadeia mova-se para o valor proposto com probabilidade $\alpha$, ou seja, sorteia-se $U \sim {\rm U}(0,1)$ e
\[ \theta_{t} = \left\{{ll} \theta_* &  \mbox{ se } u < \alpha \\ \theta_{t-1} &  \mbox{ se } u \geq \alpha \right. . \]
...
Na definição da função usaremos agora os argumentos `n` e `SQ` para evitar 
...
sendo considerados um período de ``aquecimento'' da cadeia (*burn-in*);
(ii) a distribuição $q(\cdot|\cdot)$ deve ser calibrada (no exemplo pela escolha do valor argumento `a`)
...
A este procedimento denomina-se raleamento (*thinning*); (iv) uma parametrização adequada pode assegurar uma melhor
...
Com estes pontos em mente o algorítmo `MCMC` anterior deve ser adaptado e calibrado (por tentativa e erro na especificação de `a` para atingir a taxa aceitação recomendada.
