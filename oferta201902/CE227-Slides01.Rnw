...
\[ \nu = n1+n2-2 \;\;{\rm e}\;\; S =\hat{\sigma}= \sqrt{\frac{(n_1-1)S_1^2 + (n_2-1)S_2^2}{n_1 + n_2 - 2}} \]
\end{frame}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item \red{Amostras independentes}
  \begin{itemize}
    \item \red{Variâncias iguais}
...
\begin{frame}[fragile]
  ...
  Sob modelo com variância comum:
\vspace{0.5cm}
...
\begin{frame}[fragile]
  ...
  Sob modelo com variâncias distintas:
\vspace{0.5cm}
  \red{Função de verossimilhança:}
<<echo=TRUE>>=
...
<<echo=TRUE>>=
fit2 <- mle(ll2, 
            start=list(mu=110, theta=0, lsigma1=log(10), lsigma2=log(10)), 
           fixed=list(am1 = mandible$fem, am2 = mandible$male))
@
\end{frame}
...
\begin{frame}
  \frametitle{Inferência pela verossimilhança}
\red{Inferências sobre parâmetro de interesse:}
<<results="hide",fig.width=8, fig.height=2.5, out.width = "0.99\\textwidth">>=
...
\begin{frame}[fragile]
  \frametitle{Comparando modelos pelas verossimilhanças}
...
<<echo=TRUE>>=
fit0 <- lm(mandible~1, data=mand)
m1 <- lm(mandible~sex, data=mand) ## equivalente a fit1
...
\begin{frame}[fragile]
  \frametitle{Verossimilhanças penalizadas}
...
OBS: mais apropriadas para modelos \textit{não encaixados}
...
Obtenção de (amostras da) distribuição \red{posteriori} para a diferença de médias
